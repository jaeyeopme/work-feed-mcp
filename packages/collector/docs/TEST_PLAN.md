...
- request variables에는 `request.paging.offset/count`와 optional `request.userQuery` 포함
- 기본 live page size는 50이고 CLI bound는 page size 최대 50, max pages 최대 5
...
파일: `tests/test_live_smoke.py` 또는 CLI opt-in live smoke invocation
...
- visitor-token bootstrap이 실패하고 selected credential/session-backed mode가 필요한 경우 local credential/session reference
...
3. `test_live_collects_nonzero_jobs_when_prerequisites_available`
   - visitor-token bootstrap 또는 credential/session prerequisites가 있을 때만 실행
